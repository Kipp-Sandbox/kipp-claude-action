...
describe("generateSummary", () => {
  ...
  test("disables thinking so non-thinking models do not 400", async () => {
    let capturedOptions: Record<string, unknown> | undefined;
  ...
  test("suppresses error result output and flags failure", async () => {
    const apiError =
      'API Error: 400 {"message":"adaptive thinking is not supported on this model"}';

    mock.module("@anthropic-ai/claude-agent-sdk", () => ({
      query: mock(async function* () {
        // A non-success result surfaces the provider error as an assistant
        // text block; it must not become the summary.
        yield {
          type: "assistant",
